...
router.post("/getall",fetchuser,async (req,res)=>{
        ...
        try{
            ...
            (SELECT a.testTime,a.timeOfTest,a.speed,a.accuracy from ${language}Table a WHERE a.testTime=15 ORDER BY a.speed DESC LIMIT 50)
            `)
            final.time30=await query(`
            (SELECT a.testTime,a.timeOfTest,a.speed,a.accuracy from ${language}Table a WHERE a.testTime=30 ORDER BY a.speed DESC LIMIT 50)
            `)
            final.time60=await query(`
            (SELECT a.testTime,a.timeOfTest,a.speed,a.accuracy from ${language}Table a WHERE a.testTime=60 ORDER BY a.speed DESC LIMIT 50)
            `)
            final.time120=await query(`
            (SELECT a.testTime,a.timeOfTest,a.speed,a.accuracy from ${language}Table a WHERE a.testTime=120 ORDER BY a.speed DESC LIMIT 50)
            `)
